...
{
    fprintf(stderr, "build GENH from decompressed Ben10 PS2 dumps\n");
    ...
    /* input file name 4 (WAVE data) */
    const char *datafile01_name = NULL;
    CHECK_ERROR(argc != 5, "usage: exe dump01.bin dump00.bin dumpUC00.bin dumpUC01.bin");

    indexfile_name = argv[1];
    headerfile_name = argv[2];
    datafile00_name = argv[3];
    datafile01_name = argv[4];
    ...
    FILE *datafile01 = fopen(datafile01_name, "rb");
    CHECK_ERRNO(NULL == datafile01, "input file 4 open failed");
    ...
    {
        uint32_t item_type = get_32_le_seek(offset, indexfile);
        offset += 4;
        switch (item_type)
        {
            case 0x80000113:    /* RIFF */
            {
                long riff_offset = get_32_le_seek(offset,indexfile);
            ...
            case 0x80000001:    /* ??? */
            case 0x80000000:    /* ??? */
                while (offset < file_size && 0 == (get_32_le_seek(offset,indexfile) & 0x80000000))
                    offset +=4;
                break;
            default:
                fprintf(stderr,"unknown type %08"PRIx32" at %08x\n",item_type, offset);
    ...
    )
{
            ...
            case UINT32_C(0x666d7420):  /* fmt */
                ...
                switch ( get_16_le_seek(chunk_offset + 8, headerfile) )
                {
                    ...
                    //case 0x1:
                        break;
                    default:
                        CHECK_ERROR(1, "only expecting codec id 0x600");
                        break;
                }
    ...
    CHECK_ERROR(
        !sample_count_found || -1 == name_offset || !fmt_found || !data_info_found,
    ...
    /* start_offset */
    write_32_le(0x38, &genh_head[0x1c]);
    ...
    switch (codec_id)
    {
        case 0x600:
            ...
            {
                write_32_le(0x400, &genh_head[0x08]);
            }
            break;
        case 0x1:
    ...
    put_bytes(outfile, genh_head, 0x38);

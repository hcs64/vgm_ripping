...
int main(int argc, char **argv)
{
    ...
    CHECK_ERROR( 0 != memcmp("BGIB", &header_buf[0x00], 4), "BGIB header chunk not found" );
    
    long data_start = read_32_be(&header_buf[0x04]) + 0x10;
    CHECK_ERROR( 0x200 != data_start, "unexpected header size");

    CHECK_ERROR( 0xA3 != read_32_be(&header_buf[0x08]), "bad signature at 0x08" );
    CHECK_ERROR( 1 != read_32_be(&header_buf[0x0c]), "bad signature at 0x0c" );
    CHECK_ERROR( 1 != header_buf[0x10], "bad signature at 0x10" );
    CHECK_ERROR( 0 != memcmp("\0edfedermeyer\0\0\0", &header_buf[0x50], 16), "Federmeister's signature not found" );
...

    {
        ...
        {
            if (read_32_be(&header_buf[offset]) != 0)
            {
                printf("%02lx: %08"PRIx32": ", (unsigned long)offset, read_32_be(&header_buf[offset]));
                switch (offset) 
                {
                    case 0x84:
...

    {
        ...
        for (int segment_number = 0; segment_number < segment_count; segment_number ++)
        {
            const long uncompressed_size =
                read_32_be(&header_buf[segment_info_uncomp_offsets[segment_number]]);
            const long compressed_size =
                read_32_be(&header_buf[segment_info_comp_offsets[segment_number]]);
        ...
        {
            long mstr_segment_size = read_32_be(&header_buf[0x1F8]);
        ...
        for (long uncompressed_segment_offset = 0x60; uncompressed_segment_offset < 0x84; uncompressed_segment_offset += 4)
        {
            ...
            long uncompressed_segment_size = read_32_be(&header_buf[uncompressed_segment_offset]);
